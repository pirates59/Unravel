...
const DatePage = () => {
  ...
  const renderTimes = () => {
    ...
    return times.map((time) => (
      <button
        key={time}
        className={`w-24 h-8 border rounded-lg text-sm ${
          selectedTime === time
            ? "bg-green-500 text-white"
            : "hover:bg-gray-200"
        }`}
          ...
          <div className="flex-1 p-6 flex flex-col justify-between">
            <div>
              <div className="flex justify-between items-center">
                <h2 className="text-lg font-bold mb-4">Date and Time</h2>
              ...
              {/* Calendar */}
              <div className="grid grid-cols-7 gap-2 mb-4">{renderDaysInMonth()}</div>

              {/* Times */}
              <div className="flex space-x-2">{renderTimes()}</div>
            </div>
